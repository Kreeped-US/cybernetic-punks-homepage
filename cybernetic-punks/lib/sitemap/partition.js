...
export function partitionEligible(eligible) {
  const dmz = [], dmzBuilds = [], intel = [], entities = [], wardogs = [];
  ...
  if (dmz.length + dmzBuilds.length + intel.length + entities.length + wardogs.length !== eligible.length) {
    throw new Error('[sitemap] partition: union != eligible set');
  }
  return { dmz, dmzBuilds, intel, entities, wardogs };
}
...
export function assertPartition(eligible) {
  const { dmz, dmzBuilds, intel, entities, wardogs } = partitionEligible(eligible); // throws on unassignable game/type + union-size mismatch
  const seen = new Set();
  for (const bucket of [dmz, dmzBuilds, intel, entities, wardogs]) {
