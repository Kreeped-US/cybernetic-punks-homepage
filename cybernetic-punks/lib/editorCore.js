...
- SECURITY/SAFETY EXCEPTION - name the phenomenon, not the person: naming individual users by their handle is fine and encouraged for ORDINARY public content (bug reports, feature requests, build/strategy discussion, squad-finder/LFG posts, community plugs, general opinions) - it grounds your reporting in the real community. But you MUST NOT attach a real username/handle to an individual's SECURITY or SAFETY situation - an account hack/compromise, being doxxed, stalking or harassment they are experiencing, or any personal-safety incident. Report the phenomenon WITHOUT the handle: e.g. "a player reported a name-change account hack on login - secure your accounts" NOT "u/someone was hacked." The intel is identical without the name; dropping the handle avoids amplifying a real person's vulnerability on a public, indexed page they cannot control. This applies ONLY to security/safety situations - keep naming users everywhere else.
...
// HEADLINE_RULES now lives in ./headlineRules.js (imported above). It is shared by
// all five long-form editor prompts below AND by the headline-rewrite pass, so both
...
{{kit:seasonContext.ghostLandscape}}

RANKED MODE IS LIVE: Track ranked-specific sentiment closely.{{kit:seasonContext.rankedNote.ghost}}
  ...
  try {
    ...
    if (citeStore) {
      output += '\n\n=== SOURCE CITATION -- STORE ROWS (REQUIRED) ===\n' +
        ...
        'STRICT: cite ONLY ids that literally appear as a bracketed tag above, copied exactly. The ids are arbitrary labels -- NOT sequential, NOT guessable. NEVER invent, guess, or construct an id (do not assume e.g. "WS1" or "CS30" exists because it looks plausible); an id you did not see is rejected and flags the article. If no tagged row supports a claim, leave it uncited.\n' +
