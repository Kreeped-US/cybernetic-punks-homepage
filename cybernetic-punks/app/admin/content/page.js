...
// Workflow panels (Drafts, Source Review, GSC Review, Demand Check) moved out of this page in the
// 2026-09-14 admin reorg: Drafts + Source Review -> /admin/review; GSC + Demand -> /admin/seo. This
// page is now the reference-data CRUD editor (+ a collapsed Editorial QA), not the daily workhorse.
...
export default function AdminContentPage() {
  // Auth now comes from the shared shell (app/admin/layout.js provides it + the gate);
  // this page only renders once authed, and reads the validated password from context.
  const auth = useAdminAuth();
  const password = auth.password;
  const [activeTab, setActiveTab]         = useState('editor_directives');
  const [qaOpen, setQaOpen]               = useState(false); // Editorial QA collapsed by default (lazy-mounted)
  ...
  // the Bridge launcher) opens straight to that tab. Read once on mount; no-op for an
  // unknown/absent tab. window is client-only, so this lives in an effect, not render.
  useEffect(() => {
    try {
  ...
  useEffect(() => { if (auth.authed) loadTable(activeTab); }, [auth.authed, activeTab, loadTable]);
  ...
  return (
    <div style={{ minHeight: '100vh', background: S.bg, color: S.text, fontFamily: 'Rajdhani, sans-serif' }}>
      ...
      <div style={{ padding: '24px 32px 0', borderBottom: '1px solid ' + S.border }}>
        ...
        {/* EDITORIAL QA -- editors deprioritized, so this is collapsed by default and the panels
            are LAZY-MOUNTED (they only fetch when opened), keeping this page light. The daily
            workflow (Drafts, Source Review) moved to /admin/review; SEO tools to /admin/seo. */}
        <div style={{ margin: '4px 0 24px' }}>
          <button
            onClick={() => setQaOpen((v) => !v)}
            style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'transparent', border: '1px solid ' + S.border, borderRadius: 4, color: S.muted, fontFamily: 'Share Tech Mono, monospace', fontSize: 10, letterSpacing: 2, padding: '8px 14px', cursor: 'pointer' }}
          >
            <span style={{ color: '#9b5de5' }}>{qaOpen ? '−' : '+'}</span> EDITORIAL QA <span style={{ color: 'rgba(255,255,255,0.25)' }}>&middot; quality metrics + alerts</span>
          </button>
          {qaOpen && (
      ...
      <div style={{ padding: '28px 32px' }}>
        ...
        {isDirectives && (
          <div style={{ background: 'rgba(255,45,85,0.03)', border: '1px solid rgba(255,45,85,0.12)', borderLeft: '3px solid #ff2d55', borderRadius: 8, padding: '16px 20px', marginBottom: 24 }}>
            ...
            <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 14, color: 'rgba(255,255,255,0.4)', lineHeight: 1.6 }}>
              Queue a topic for any editor. Use <span style={{ color: '#00f5ff' }}>creator_spotlight</span> type to feed vetted creator/community news the editor writes up and tags. Use <span style={{ color: '#c8d4e0' }}>discourse</span> (editor <span style={{ color: '#c8d4e0' }}>VANTAGE</span>) to queue a curated take for a discourse DRAFT -- then hit <span style={{ color: '#c8d4e0' }}>GENERATE</span> on the row here (or on <span style={{ color: '#00ff88' }}>/admin/review</span>) to write the draft, reviewed + approved in the Drafts panel on /admin/review; nothing publishes until you approve. Set <span style={{ color: '#00f5ff' }}>Scheduled For</span> to fire on a future date (your local time), or leave blank for the next cron cycle. Status auto-updates to <span style={{ color: '#00ff88' }}>consumed</span> after use.
            </div>
          </div>
        )}
