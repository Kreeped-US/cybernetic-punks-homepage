...
// Pulls curated builds + DEXTER articles from Supabase
...
const WEAPON_CATEGORIES = [
  ...
  },
  {
    ...
    picks: [
      ...
      { name: 'CE Tactical', tier: 'B', ammo: 'Medium Rounds', note: 'Rook starter. Prestige mod grants invis on kill+reload.' },
    ]
  },
];
...
export default async function BuildsPage() {
  // Fetch curated builds
  const { data: builds } = await supabase
    .from('builds')
    .select('*')
    .order('updated_at', { ascending: false });

  // Fetch DEXTER articles (build content)
  ...
  // Group DEXTER articles by shell
  ...
  (dexterArticles || []).forEach(article => {
    ...
    for (const shell of shellNames) {
      if (tags.includes(shell) || headline.includes(shell)) {
        if (!articlesByShell[shell]) articlesByShell[shell] = [];
        articlesByShell[shell].push(article);
        matched = true;
        break;
      }
    }
    if (!matched) generalArticles.push(article);
  });

  const totalArticles = (dexterArticles || []).length;
    ...
    <main style={{ background: BLACK, minHeight: '100vh', color: '#ffffff' }}>
      ...
      <section style={{
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{
            ...
            <Stat label="BUILDS ANALYZED" value={totalArticles} color={DEXTER_ORANGE} />
            <Stat label="RUNNER SHELLS" value="6" color={CYAN} />
            <Stat label="WEAPONS" value="28" color={RED} />
      ...
      <section style={{
        ...
        <div style={{
          ...
          <AmmoTag name="Medium Rounds" color="#aaaaaa" />
      ...
      {/* ─── CURATED BUILDS (from builds table) ──────── */}
      {builds && builds.length > 0 && (
        <section style={{
          ...
          <p style={{
            ...
            Hand-graded loadouts rated by effectiveness, versatility, and skill floor.
          </p>
            ...
            {builds.map(build => (
              <div key={build.id} style={{
                ...
                <div style={{
                  ...
                  fontSize: '28px',
                  fontWeight: 700,
                  color: build.grade === 'S' ? RED : build.grade === 'A' ? DEXTER_ORANGE : CYAN,
                  opacity: 0.3,
                  lineHeight: 1,
                }}>
                  {build.grade}
      ...
      {/* ─── SHELL SECTIONS (DEXTER articles by shell) ── */}
      {SHELLS.map(shell => {
                      ...
                      <div style={{
                        ...
                        {article.ce_score && (
                          <span style={{
                            ...
                            fontSize: '11px',
                            fontWeight: 700,
                            color: article.ce_score >= 80 ? RED : article.ce_score >= 60 ? DEXTER_ORANGE : CYAN,
                          }}>
                            CE:{article.ce_score}
                          </span>
                        )}
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{
        __html: JSON.stringify({
          ...
          mainEntity: {
            ...
            itemListElement: SHELLS.map(function(shell, i) {
              return {
                '@type': 'ListItem',
                position: i + 1,
