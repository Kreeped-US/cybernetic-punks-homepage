...
const TIERS = [
  {
    name: 'Bronze',   color: '#cd7f32', bg: 'rgba(205,127,50,0.07)',  border: 'rgba(205,127,50,0.25)',
  ...
  {
    name: 'Silver',   color: '#aaaaaa', bg: 'rgba(170,170,170,0.05)', border: 'rgba(170,170,170,0.2)',
    zones: 'LOW-STAKES', zonesColor: '#00ff88',
    desc: 'Consistent runs. Efficient looting and smart exfil choices define this tier.',
    rewards: ['Ranked Emblem', 'Player Background'],
  },
  {
    name: 'Gold',     color: '#ffd700', bg: 'rgba(255,215,0,0.07)',   border: 'rgba(255,215,0,0.25)',
    zones: 'LOW-STAKES', zonesColor: '#00ff88',
  ...
  {
    name: 'Platinum', color: '#00f5ff', bg: 'rgba(0,245,255,0.05)',   border: 'rgba(0,245,255,0.2)',
  ...
  {
    name: 'Diamond',  color: '#66ccff', bg: 'rgba(102,204,255,0.05)', border: 'rgba(102,204,255,0.2)',
    zones: 'HIGH-STAKES', zonesColor: '#ff0000',
    desc: 'Elite extraction crews. Precision, coordination, and maximum gear ante.',
    rewards: ['Ranked Emblem', 'Gun Style'],
  },
  {
    name: 'Pinnacle', color: '#cc44ff', bg: 'rgba(204,68,255,0.07)',  border: 'rgba(204,68,255,0.25)',
    zones: 'HIGH-STAKES', zonesColor: '#ff0000',
...
const FAQS = [
  ...
  { q: 'What are the ranked rewards?',                     a: 'Bronze: Ranked Emblem. Silver: Emblem + Player Background. Gold: Emblem + Destroyer Shell Style + Title. Platinum/Diamond: Emblem + Gun Style. Pinnacle: Emblem + Gun Style + Title. Rewards are based on highest rank achieved during the season.' },
...
// ─── ACCURATE IN-GAME RANK EMBLEMS ──────────────────────────
// Each emblem: outer ring + 4 crosshair lines piercing through ring + inner ring + unique center
function RankEmblem({ tier, size = 80, glow = false }) {
  ...
  var outerR  = s * 0.40;
  var innerR  = s * 0.26;
  var lineGap = s * 0.08; // gap between center and line start
  var lineOut = s * 0.10; // how far line extends past outer ring
  var dotR    = s * 0.055;

  // Line endpoints — pierce through the outer ring
  var lines = [
    { x1: c, y1: c - innerR + lineGap,      x2: c, y2: c - outerR - lineOut },      // top
    { x1: c, y1: c + innerR - lineGap,      x2: c, y2: c + outerR + lineOut },      // bottom
    { x1: c - innerR + lineGap, y1: c,      x2: c - outerR - lineOut, y2: c },      // left
    { x1: c + innerR - lineGap, y1: c,      x2: c + outerR + lineOut, y2: c },      // right
  ];

  // Center design — unique per tier
  var center = null;
  var n = tier.name;
  ...
  if (n === 'Bronze') {
    ...
    // 2 dots stacked vertically
    var sp2 = s * 0.07;
    center = (
      <>
        <circle cx={c} cy={c - sp2} r={dotR} fill={color} opacity="0.95" />
        <circle cx={c} cy={c + sp2} r={dotR} fill={color} opacity="0.95" />
      </>
    );
  } else if (n === 'Gold') {
    // 3 dots stacked vertically
    var sp3 = s * 0.075;
    center = (
      <>
        <circle cx={c} cy={c - sp3} r={dotR} fill={color} opacity="0.95" />
        <circle cx={c} cy={c}       r={dotR} fill={color} opacity="0.95" />
        <circle cx={c} cy={c + sp3} r={dotR} fill={color} opacity="0.95" />
      </>
    );
  } else if (n === 'Platinum') {
    // 4-petal clover — 4 circles at N/S/E/W
    var sp4 = s * 0.075;
    var pr  = dotR * 1.15;
    center = (
      <>
        <circle cx={c}       cy={c - sp4} r={pr} fill={color} opacity="0.9" />
        <circle cx={c}       cy={c + sp4} r={pr} fill={color} opacity="0.9" />
        <circle cx={c - sp4} cy={c}       r={pr} fill={color} opacity="0.9" />
        <circle cx={c + sp4} cy={c}       r={pr} fill={color} opacity="0.9" />
      </>
    );
  } else if (n === 'Diamond') {
    // Larger 4-petal, slightly more spread, with tiny center dot
    var sp5 = s * 0.088;
    var pr5 = dotR * 1.2;
    center = (
      <>
        <circle cx={c}       cy={c - sp5} r={pr5} fill={color} opacity="0.9" />
        <circle cx={c}       cy={c + sp5} r={pr5} fill={color} opacity="0.9" />
        <circle cx={c - sp5} cy={c}       r={pr5} fill={color} opacity="0.9" />
        <circle cx={c + sp5} cy={c}       r={pr5} fill={color} opacity="0.9" />
        <circle cx={c}       cy={c}       r={dotR * 0.6} fill={color} opacity="0.7" />
      </>
    );
  } else if (n === 'Pinnacle') {
    // Most ornate — 4 large petals + 4 diagonal smaller petals + center
    var sp6a = s * 0.09;
    var sp6b = s * 0.075;
    var pr6a = dotR * 1.25;
    var pr6b = dotR * 0.85;
    ...
    center = (
      <>
        {/* Cardinal petals */}
        <circle cx={c}         cy={c - sp6a} r={pr6a} fill={color} opacity="0.95" />
        <circle cx={c}         cy={c + sp6a} r={pr6a} fill={color} opacity="0.95" />
        <circle cx={c - sp6a}  cy={c}        r={pr6a} fill={color} opacity="0.95" />
        <circle cx={c + sp6a}  cy={c}        r={pr6a} fill={color} opacity="0.95" />
        {/* Diagonal petals */}
  ...
  return (
    <svg width={s} height={s} viewBox={'0 0 ' + s + ' ' + s} style={{ flexShrink: 0, filter: glow ? 'drop-shadow(0 0 ' + (s * 0.1) + 'px ' + color + '99)' : 'none', overflow: 'visible' }}>
      ...
      {lines.map(function(l, i) {
        return <line key={i} x1={l.x1} y1={l.y1} x2={l.x2} y2={l.y2} stroke={color} strokeWidth={s * 0.035} strokeLinecap="round" opacity="0.9" />;
      })}
      {/* Unique center */}
      {center}
    </svg>
  ...
  return (
    <div style={{ display: 'flex', gap: 4 }}>
      {['III', 'II', 'I'].map(function(d) {
        return (
          <span key={d} style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 8, color: color, background: color + '18', border: '1px solid ' + color + '40', borderRadius: 2, padding: '2px 5px', letterSpacing: 1 }}>{d}</span>
        );
      })}
    </div>
  );
...
function Countdown() {
  ...
  return (
    <div>
        ...
        {[{ val: parts.d, label: 'DAYS' }, { val: parts.h, label: 'HOURS' }, { val: parts.m, label: 'MIN' }, { val: parts.s, label: 'SEC' }].map(function(unit, i) {
          return (
            <div key={unit.label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontFamily: 'Orbitron, monospace', fontSize: 'clamp(1.6rem, 4vw, 2.4rem)', fontWeight: 900, color: '#00f5ff', lineHeight: 1, background: 'rgba(0,245,255,0.05)', border: '1px solid rgba(0,245,255,0.2)', borderRadius: 6, padding: '10px 14px', minWidth: 62, textShadow: '0 0 20px rgba(0,245,255,0.3)' }}>
                  {unit.val}
                </div>
      ...
      <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 9, color: 'rgba(255,255,255,0.2)', letterSpacing: 2, marginTop: 10, textAlign: 'center' }}>
    ...
    <main style={{ backgroundColor: '#030303', minHeight: '100vh', color: '#fff', paddingTop: 80, overflowX: 'hidden' }}>
      ...
      <div style={{ background: 'rgba(255,136,0,0.05)', borderBottom: '1px solid rgba(255,136,0,0.12)', padding: '10px 24px', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 13, color: 'rgba(255,255,255,0.4)', lineHeight: 1.4 }}>
          ...
          Bungie is actively looking for feedback to continue shaping Ranked. Stats, thresholds, and systems are subject to change.
        </span>
      </div>

      {/* ── HERO ─────────────────────────────────────── */}
      <section style={{ position: 'relative', overflow: 'hidden', padding: '56px 24px 64px' }}>
        {/* Site-consistent grid background */}
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
            <span style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 9, color: '#00f5ff', background: 'rgba(0,245,255,0.08)', border: '1px solid rgba(0,245,255,0.25)', borderRadius: 3, padding: '4px 12px', letterSpacing: 2 }}>LAUNCHING SOON</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 48, alignItems: 'center' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                ...
                <Link href="/meta" style={{ padding: '10px 20px', background: 'rgba(0,245,255,0.05)', border: '1px solid rgba(0,245,255,0.18)', borderRadius: 5, textDecoration: 'none', fontFamily: 'Share Tech Mono, monospace', fontSize: 10, color: '#00f5ff', letterSpacing: 2 }}>
                  [N] RANKED META &rarr;
                </Link>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
              <Countdown />
              {/* Vertical emblem parade matching the game's sidebar layout */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, flexWrap: 'wrap' }}>
                {TIERS.map(function(tier) {
                  return <RankEmblem key={tier.name} tier={tier} size={38} />;
                })}
              </div>
            </div>
          </div>
            ...
            <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '22px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 16, paddingBottom: 16, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div>
                  ...
                  <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 13, color: 'rgba(255,255,255,0.4)', lineHeight: 1.5 }}>Mandatory passes for Runners entering Ranked play. Each Holotag adds to the crew's score target based on rarity — higher rarity tags also provide greater scoring capacity.</div>
                </div>
              </div>
              {[
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ background: '#0a0a0a', border: '1px solid rgba(0,255,136,0.12)', borderRadius: 8, padding: '18px 20px' }}>
                ...
                <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 13, color: 'rgba(255,255,255,0.4)', lineHeight: 1.5 }}>Available to all Runners across every rank. Standard extraction risk with controlled competition density. The foundational Ranked experience.</div>
              </div>
              ...
              <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 6, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: 14 }}>↻</span>
                <span style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 13, color: 'rgba(255,255,255,0.35)' }}>Ranked zones rotate each week.</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ padding: '0 24px 64px', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 8 }}>
          {[
            ...
            { icon: '◎', color: '#00f5ff', title: 'HOLOTAG THEFT',                  body: 'Holotags stolen from defeated Runners are yours to keep — they contribute score and capacity to your crew.' },
          ].map(function(item, i) {
            return (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 10, marginBottom: 12 }}>
          <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, padding: '20px' }}>
            ...
            <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 13, color: 'rgba(255,255,255,0.4)', lineHeight: 1.7 }}>
              Based on the <strong style={{ color: '#fff' }}>highest rank you achieved during the season</strong> — not your final standing. Every tier earns you a Ranked Emblem specific to that rank, plus additional rewards at higher tiers.
            </div>
          </div>
          <div style={{ background: '#0a0a0a', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
