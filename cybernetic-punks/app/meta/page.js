...
const TIER_STYLES = {
  ...
  D: { bg: 'rgba(255,255,255,0.02)', color: 'rgba(255,255,255,0.25)', border: 'rgba(255,255,255,0.04)', label: 'AVOID' },
};
...
export default async function MetaPage() {
  // Fetch live tier data from Supabase
  let tiers = {};
  ...
  try {
    ...
    console.error('MetaPage recent posts error:', err);
  }

  // Only show tiers that have items
  const activeTiers = TIER_ORDER.filter(t => tiers[t] && tiers[t].length > 0);
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto 48px', padding: '0 24px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 8 }}>
          ...
          <div style={{
            ...
            {lastUpdated ? 'LAST UPDATED ' + lastUpdated.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase() : 'AUTO-UPDATED EVERY 6 HOURS'}
          </div>
        </div>
        <p style={{
        ...
        {activeTiers.length === 0 ? (
          <div style={{
            ...
            <div style={{
              ...
              TIER DATA LOADING • CHECK BACK SHORTLY
            </div>
          </div>
        ) : (
