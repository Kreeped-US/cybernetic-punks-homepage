// components/game/ThemedGameFooter.js
// GENERALIZED themed footer -- the config-driven equivalent of the bespoke
// components/wardogs/WardogsFooter.js, so any game can opt into a Wardogs-quality
// themed footer (theme color + official logo + backdrop) by SUPPLYING CONFIG rather
// than cloning WardogsFooter. Wardogs itself is untouched and remains the reference.
//
// STEP 0 / CAPABILITY ONLY: components/Footer.js renders this ONLY when a game's
// config declares footer.themed.enabled === true. No game declares it yet, so this
// ships dormant -- every hub keeps its exact current footer. A later rollout brief
// flips one game on per window.
//
// ── CONFIG CONTRACT (lib/games/<game>.js) ──────────────────────────────────────
// Add an OPTIONAL block under the existing `footer` object. It is additive: absent
// -> the generic network footer renders byte-identical to today.
//
//   footer: {
...
//                                         //   || config.theme.primary || a neutral.
//       backdrop: {                       // optional. Omit -> solid themed bg, no image.
//         src: '/images/games/<game>-hero.jpg',
...
//   }
//
// ASSET NOTE (rollout-time, not this brief): a logo with a BAKED, NON-TRANSPARENT
// background (e.g. dmz's /images/DMZ/dmzlogo.webp is a solid-black 1:1 square) will
// render as a visible box on a non-black footer. Such an asset needs a transparent
// re-export BEFORE that game sets enabled:true. This component does not assume
// transparency; it just renders the src it is given.
//
// Server-safe (Link + img only, no hooks) -- mirrors WardogsFooter so it can be
// dispatched from the client Footer exactly as WardogsFooter already is.

import Link from 'next/link';
import { DISCORD_INVITE } from '@/lib/socialLinks';

export default function ThemedGameFooter({ config }) {
  const cfg = config || {};
  const fcfg = cfg.footer || {};
  const th = fcfg.themed || {};
  const gtheme = cfg.theme || {};

  // Accent: explicit override, else the game's own theme accent/primary, else neutral.
  const accent = th.color || gtheme.accent || gtheme.primary || '#9aa3ad';
  const bg = gtheme.bgPage || '#0b0d10';
  ...
  const linkStyle = { color: 'rgba(255,255,255,0.72)', textDecoration: 'none', fontSize: 13, fontWeight: 500 };
  ...
  return (
    <footer style={{ position: 'relative', overflow: 'hidden', background: bg, borderTop: '1px solid #1d2026', marginTop: 40 }}>
      ...
      <div style={{ position: 'relative', maxWidth: 1120, margin: '0 auto', padding: '44px 24px 26px' }}>
            ...
            {explore.length > 0 && (
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
                  {explore.map((l) => (
                    <Link key={l.href} href={l.href} className="tg-foot-link" style={linkStyle}>{l.label}</Link>
                  ))}
                </div>
              </div>
            )}
        ...
        {/* network line + legal (back-to-network link preserved, themed accent) */}
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: 18, display: 'flex', gap: 16, flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'baseline' }}>
          <Link href="/" className="tg-foot-link" style={{ fontSize: 12, fontWeight: 700, letterSpacing: 0.3, color: accent, textDecoration: 'none' }}>
      ...
      <style>{'.tg-foot-link:hover{color:#fff !important}'}</style>
    </footer>
  );
}
